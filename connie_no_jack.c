...
AudioBuffer *init_audio_buffer(size_t num_channels, size_t buffer_size) {
    AudioBuffer *buffer = malloc(sizeof(AudioBuffer));
    buffer->num_channels = num_channels;
    buffer->buffer_size = buffer_size;
    buffer->buffers = malloc(num_channels * sizeof(float *));
    for (size_t i = 0; i < num_channels; i++) {
        buffer->buffers[i] = calloc(buffer_size, sizeof(float));
    }
    return buffer;
}
...
void process_audio(AudioBuffer *buffer) {
    for (size_t frame = 0; frame < buffer->buffer_size; frame++) {
        for (size_t ch = 0; ch < buffer->num_channels; ch++) {
            buffer->buffers[ch][frame] = sinf(2.0 * M_PI * 440.0 * frame / 44100.0f); // Example sine wave
        }
    }
}
#include <stdlib.h>
...
static  *jack_client = NULL;
static  *jack_midi_port;
static  *jack_audio_port_l;
static  *jack_audio_port_r;
...
// ******************************************
//
static int rt_process_cb( size_t nframes, void *void_arg ) {
...
static int jack_srate_cb( size_t nframes, void *arg ) {
...
int main(int argc, char *argv[]) {
    AudioBuffer *buffer = init_audio_buffer(2, 512); // Stereo buffer
    for (int i = 0; i < 10; i++) {
        process_audio(buffer);
        printf("Cycle %d - Left Channel Sample 0: %f\n", i, buffer->buffers[0][0]);
    }
